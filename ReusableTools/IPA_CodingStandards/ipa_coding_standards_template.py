#!/usr/bin/env python3
"""
IPA Coding Standards Template - V4.0 Professional Audit Report

MAJOR IMPROVEMENTS IN V4:
1. ✓ Logical sheet ordering: Dashboard → Metrics → Detailed Analysis → Action Items → Flow
2. ✓ Standardized navigation across ALL sheets (row 1)
3. ✓ Grid-aligned Executive Dashboard with consistent KPI layout
4. ✓ Metrics Calculation transparency sheet with formulas
5. ✓ Filterable Detailed Analysis table with Action IDs
6. ✓ Action ID traceability system (ACT-001, ACT-002, etc.)
7. ✓ Structured Action Items table sorted by severity
8. ✓ Bidirectional hyperlinks between violations and fixes
9. ✓ Standardized severity colors across all sheets
10. ✓ Dynamic chart data (no hardcoded values)

SHEET ORDER (Professional Audit Report Flow):
1. 📊 Executive Dashboard - High-level summary for leadership
2. 📐 Metrics Calculation - Formula transparency and methodology
3. 📐 Detailed Analysis - Full violation list with Action IDs and hyperlinks
4. ✅ Action Items - Remediation checklist with traceability
5. 🔄 Process Flow - Visual diagram with complexity metrics

Version: 4.0
Created: 2026-03-08
Purpose: Professional stakeholder presentation with full traceability
"""
...
def generate_report(ipa_data):
    """Generate professional audit report with all V4 improvements"""
    ...
    # IMPROVEMENT 1: Create sheets in logical audit report order
    # Dashboard → Metrics → Detailed Analysis → Action Items → Flow
...
def create_executive_dashboard(wb, ipa_data):
    ...
    # IMPROVEMENT 2: Add navigation links in row 2 (standardized)
    add_navigation_links(ws, wb, '📊 Executive Dashboard')
    
    # Hero section (now row 2 after navigation in row 1)
    ...
    ws.merge_range('A2:Q2', '📊 CODING STANDARDS DASHBOARD', hero_format)
    ws.set_row(1, 45)
    
    # Metadata bar (now row 3)
    client = ipa_data.get('client_name', 'Client')
    rice_item = ipa_data.get('rice_item', 'Project')
    ...
    ws.merge_range('A3:Q3', f"{client} • {rice_item} | Generated: {datetime.now().strftime('%B %d, %Y at %I:%M %p')}", meta_format)
    ws.set_row(2, 22)
    ...
    row = 4  # Start after navigation (row 1), title (row 2), metadata (row 3)
    ...
    # IMPROVEMENT 3: Severity Risk Indicator Banner (aligned to grid B-Q)
    ...
    ws.merge_range(row, 1, row, 16, f'{risk_status}', risk_banner_format)  # Columns B-Q
    ws.set_row(row, 30)
    row += 2  # Skip duplicate Technical Debt section (IMPROVEMENT 2: removed)
    
    # === MIDDLE SECTION: CHARTS (Grid-Aligned) ===
    # Left chart: Column B (1), Right chart: Column K (10)
    # Both charts: 480x360 pixels, aligned to grid
    ...
    # Left section header: Columns B-J (1-9)
    ws.merge_range(row, 1, row, 9, '📊 VIOLATION SEVERITY DISTRIBUTION', section_header)
    
    # Right section header: Columns K-Q (10-16)
    ws.merge_range(row, 10, row, 16, '✅ CODE QUALITY PASS RATE', section_header)
    ws.set_row(row, 28)
    row += 1
    
    # Store chart_row for data references
    chart_row = row
    
    # Create Clustered Column Chart (Left)
    column_chart = wb.add_chart({'type': 'column'})
    # Note: data_row_start will be defined after charts
    ...
    # Insert at column B (1)
    ws.insert_chart(row, 1, column_chart)
    
    # Create Donut Chart (Right)
    ...
    # Insert at column K (10)
    ws.insert_chart(row, 10, donut_chart)
    
    # Advance row past charts (22 rows for 360px height)
    row += 22
    
    # === SEVERITY DATA TABLE (Below Charts) ===
    row += 2
    ...
    ws.write(row, 1, 'Severity', data_header_format)
    ws.write(row, 2, 'Count', data_header_format)
    ws.set_row(row, 20)
    row += 1
    ...
    ws.write(row, 1, 'High Violations', data_format)
    ws.write(row, 2, high_count, data_value_format)
    row += 1
    
    ws.write(row, 1, 'Medium Violations', data_format)
    ws.write(row, 2, medium_count, data_value_format)
    row += 1
    
    ws.write(row, 1, 'Low Violations', data_format)
    ws.write(row, 2, low_count, data_value_format)
    row += 1
    
    # Write pass/fail data for donut chart
    ws.write(data_row_start - 1, 11, 'Status', data_header_format)
    ws.write(data_row_start - 1, 12, 'Count', data_header_format)
    
    ws.write(data_row_start, 11, 'Passed Checks', data_format)
    ws.write(data_row_start, 12, passed_checks, data_value_format)
    
    ws.write(data_row_start + 1, 11, 'Failed Checks', data_format)
    ws.write(data_row_start + 1, 12, total_violations, data_value_format)
    
    # Now add chart data series (after data is written)
    column_chart.add_series({
        'name': 'Violations',
        'categories': ['📊 Executive Dashboard', data_row_start, 1, data_row_start + 2, 1],
        'values': ['📊 Executive Dashboard', data_row_start, 2, data_row_start + 2, 2],
    ...
    donut_chart.add_series({
        'name': 'Code Quality',
        'categories': ['📊 Executive Dashboard', data_row_start, 11, data_row_start + 1, 11],
        'values': ['📊 Executive Dashboard', data_row_start, 12, data_row_start + 1, 12],
...
def create_action_items_enhanced(wb, ipa_data):
    ...
    # IMPROVEMENT 2: Navigation links (must be FIRST, before title)
    add_navigation_links(ws, wb, '✅ Action Items')
    
    # Modern title (now row 2 after navigation in row 1)
    ...
    ws.merge_range('A2:P2', '✅ ACTION ITEMS - ENHANCED DEVELOPER CHECKLIST', title_format)
    ws.set_row(1, 35)
    ...
    ws.merge_range('A3:P3', 'Items with impact analysis, code examples, and testing guidance', subtitle_format)
    ws.set_row(2, 20)
    ...
    for violation in ipa_data.get('violations', []):
        activity = violation.get('activity_id', violation.get('activities', ''))
...
def create_detailed_analysis_enhanced(wb, ipa_data):
    ...
    # IMPROVEMENT 2: Navigation links (must be FIRST, before title)
    add_navigation_links(ws, wb, '📐 Detailed Analysis')
    ...
    ws.merge_range('A2:J2', '📐 DETAILED ANALYSIS WITH IMPACT ASSESSMENT', title_format)
    ws.set_row(1, 35)
    ...
    ws.merge_range('A3:J3', 'Comprehensive technical analysis with code examples and testing guidance', subtitle_format)
    ws.set_row(2, 20)
    ...
    if violations:
        for idx, violation in enumerate(violations, 1):
            ...
            activity_value = violation.get('activity_id', violation.get('activities', 'N/A'))
...
def create_metrics_calculation_sheet(wb, ipa_data):
    ...
    # IMPROVEMENT 2: Add navigation links
    add_navigation_links(ws, wb, '📐 Metrics Calculation')
    ...
    ws.merge_range('A2:H2', '📐 METRICS CALCULATION METHODOLOGY', title_format)
    ws.set_row(1, 35)
    ...
    ws.merge_range('A3:H3', 'Transparency documentation for dashboard metrics', subtitle_format)
    ws.set_row(2, 20)
    ...
    row = 4  # Start after navigation (row 0), title (row 1), subtitle (row 2), and blank row (row 3)
...
def create_process_flow(wb, ipa_data):
    ...
    # IMPROVEMENT 2: Navigation links (must be FIRST, before title)
    add_navigation_links(ws, wb, '🔄 Process Flow')
    ...
    ws.merge_range('A2:L2', '🔄 PROCESS FLOW', title_format)
    ws.set_row(1, 35)
    ...
    ws.merge_range('A3:L3', 'Visual representation of process structure and complexity', subtitle_format)
    ws.set_row(2, 20)
